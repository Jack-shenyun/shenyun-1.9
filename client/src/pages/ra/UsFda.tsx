...
import {
  ...
  Flag, FileText, CheckCircle2, Clock, Send, Archive,
} from "lucide-react";
...
export default function UsFdaPage() {
  ...
  const allProjects = (projects as any[]).filter((p) => p.market === "US_FDA");
  ...
  const handleSubmit = () => {
    ...
    if (editingId) {
      updateMutation.mutate({ id: editingId, name: formData.name, market: "US_FDA" });
    } else {
      createMutation.mutate({ name: formData.name, market: "US_FDA" });
    }
  };
  ...
  return (
    <RaLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold tracking-tight">US FDA 申报项目</h2>
              <p className="text-sm text-muted-foreground">
                美国食品药品监督管理局 · 510(k) / PMA / De Novo 申报管理
              </p>
            </div>
          </div>
        ...
        <div className="grid gap-4 grid-cols-2 md:grid-cols-5">
          {[
            ...
            { label: "进行中", value: countByStatus("in_progress"), color: "text-red-600" },
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-12">
                      <div className="flex flex-col items-center gap-2 text-muted-foreground">
                        ...
                        <p className="text-sm">暂无 US FDA 申报项目</p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingId ? "编辑项目" : "新建 US FDA 申报项目"}</DialogTitle>
            <DialogDescription>创建 510(k) / PMA / De Novo 申报项目</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label>项目名称</Label>
              <Input
                placeholder="例：XX型医疗器械 510(k) 申报"
                value={formData.name}
