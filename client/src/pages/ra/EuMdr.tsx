...
export default function EuMdrPage() {
  ...
  const allProjects = (projects as any[]).filter((p) => p.market === "EU_MDR");
  ...
  const handleSubmit = () => {
    ...
    if (editingId) {
      updateMutation.mutate({ id: editingId, name: formData.name, market: "EU_MDR" });
    } else {
      createMutation.mutate({ name: formData.name, market: "EU_MDR" });
    }
  };
  ...
  return (
    <RaLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold tracking-tight">EU MDR 申报项目</h2>
              <p className="text-sm text-muted-foreground">
                欧盟医疗器械法规 · 每个项目包含 {MDR_STATS.documents} 份技术文件、{MDR_STATS.totalSections} 个章节
              </p>
            </div>
          </div>
        ...
        <div className="grid gap-4 grid-cols-2 md:grid-cols-5">
          {[
            ...
            { label: "进行中", value: countByStatus("in_progress"), color: "text-blue-600" },
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-12">
                      <div className="flex flex-col items-center gap-2 text-muted-foreground">
                        ...
                        <p className="text-sm">暂无 EU MDR 申报项目</p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingId ? "编辑项目" : "新建 EU MDR 申报项目"}</DialogTitle>
            <DialogDescription>
              创建后将自动生成 {MDR_STATS.documents} 份技术文件模板
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label>项目名称</Label>
              <Input
                placeholder="例：XX型医疗器械 EU MDR 申报"
                value={formData.name}
